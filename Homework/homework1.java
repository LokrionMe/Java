...
public class homework1 {
    public static void main(String[] args) {
         ...
         * с некоторыми вариантами справляется(?7 + 6? = 83), но некоторые нет(6? + ?7 =
         * 83),
        ...
        for (int i = arr.length - 1; i != -1; i--) {
            ...
            if (a.indexOf('?') != -1) {
                for (int j = 0; j < a.length(); j++) {
                    ...
                    if (a.charAt(j) == '?') {
                        if (i == 0) {
                            res = Character.getNumericValue(arr[2].charAt(j))
                                    - Character.getNumericValue(arr[1].charAt(j));
                        } else if (i == 1) {
                            res = Character.getNumericValue(arr[2].charAt(j))
                                    - Character.getNumericValue(arr[0].charAt(j));
                        } else {
                            res = Character.getNumericValue(arr[0].charAt(j))
                                    + Character.getNumericValue(arr[1].charAt(j));
                        }
                        if (k == 1) {
                            if (i == 0 || i == 1) {
